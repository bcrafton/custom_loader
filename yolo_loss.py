...
def iou_train(boxA, boxB, realBox):
    ...
    Calculate the IoU icn training phase, to get the box (out of N boxes per grid) responsible for ground truth box
    """
    iou1 = tf.reshape(iou_train_unit(boxA, realBox), [8, 16, 7, 7, 1])
    iou2 = tf.reshape(iou_train_unit(boxB, realBox), [8, 16, 7, 7, 1])
    return tf.concat([iou1, iou2], 4)
...
def yolo_loss(pred, label, obj, no_obj, cat):
    ...
    # pred   = [batch_size,           7, 7, 2 * 5 + 80]
    # label  = [batch_size, det_size, 7, 7, 5]
    # obj    = [batch_size, det_size, 7, 7]
    # no_obj = [batch_size, det_size, 7, 7]
    # cat    = [batch_size, det_size, 7, 7]

    ######################################

    pred = tf.reshape(pred, [8, 1, 7, 7, 2 * 5 + 80])
    ...
    # precision = TP / (TP + FP)
    TP = tf.count_nonzero(tf.greater(obj * tf.reduce_max(iou, axis=4), 0.5 * tf.ones_like(obj)))
    ...
    loss_xy1 = tf.reduce_sum(tf.square(pred_xy1 - label_xy), 4)
    loss_xy2 = tf.reduce_sum(tf.square(pred_xy2 - label_xy), 4)
    xy_loss = 5. * obj * tf.where(resp_box, loss_xy1, loss_xy2)

    ######################################

    loss_wh1 = tf.reduce_sum(tf.square(pred_wh1 - label_wh), 4)
    loss_wh2 = tf.reduce_sum(tf.square(pred_wh2 - label_wh), 4)
    ...
    pred_cat = tf.reshape(obj, [8,16,7,7,1]) * pred_cat
